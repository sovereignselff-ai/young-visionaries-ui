...
import { PlayableTriviaGame } from "@/components/playable-trivia-game";
import { useCurriculum } from "@/contexts/CurriculumContext";
import { olivier, olivierHomework } from "@/data/mockData";
...
export function StudentPortal() {
  const studentData = olivier;
  const studentHomework = olivierHomework;
  const xpPercent = (studentData.xp / studentData.xpToNext) * 100;
  const [activeGameId, setActiveGameId] = useState<string | null>(null);

  const { getStudentAssignments, updateAssignmentStatus } = useCurriculum();
  const assignments = getStudentAssignments("olivier");

  const handleGameComplete = (assignmentId: string, score: number, xp: number) => {
    updateAssignmentStatus(assignmentId, "completed", score, xp);
    setActiveGameId(null);
  };

  // Find the active game to display
  const activeGame = activeGameId ? assignments.find((a) => a.id === activeGameId) : null;
      ...
      <>
        ...
        <div className="flex flex-col gap-8 p-6 lg:p-8 relative min-h-[calc(100vh-3.5rem)]">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="scroll-m-20 text-2xl font-semibold tracking-tight text-purple-900">
                Playing Module
              </h1>
              <p className="text-muted-foreground mt-1">Focus and give it your best effort!</p>
            </div>
          ...
          <div className="max-w-2xl mx-auto w-full">
            <PlayableTriviaGame
              gameData={activeGame.gameData}
              onComplete={(score, xp) => handleGameComplete(activeGame.id, score, xp)}
            />
          </div>
        </div>
      </>
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-8 p-6 lg:p-8 relative min-h-[calc(100vh-3.5rem)]">
              ...
              <div className="flex-1 space-y-3">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-sm text-muted-foreground">{studentData.grade} Grade</span>
                  </div>
                </div>
                <div className="space-y-1.5">
                  ...
                  <Progress
                    value={xpPercent}
                    className="h-3 bg-primary/10 [&>[data-slot=progress-indicator]]:bg-xp"
                  />
        ...
        {/* Adaptive Homework Hub with Purple Styling */}
        <div>
          <h2 className="scroll-m-20 text-xl font-semibold tracking-tight mb-4 text-purple-900">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {assignments.map((assignment) => (
              <Card
                key={assignment.id}
                className="group hover:shadow-md transition-all border-purple-200 bg-purple-50 hover:border-purple-400"
              >
                <CardHeader>
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-1 text-xs text-purple-700">
                      ...
                      {assignment.status === "completed" ? "Complete" : "Pending"}
                    </div>
                  </div>
                  <CardTitle className="text-base mt-2 text-purple-900">
                    {assignment.gameData.title}
                  </CardTitle>
                  <CardDescription className="text-purple-700">
                    {assignment.gameData.questions.length} Questions
                  </CardDescription>
                </CardHeader>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {assignment.status === "pending" && (
                      <Button
                        ...
                        onClick={() => setActiveGameId(assignment.id)}
                      >
                    ...
                    {assignment.status === "completed" && (
                      <Button
                        size="sm"
                        variant="outline"
                        className="gap-1.5 border-purple-200 text-purple-900 hover:bg-purple-100"
                      >
            ...
            {studentHomework.map((task) => {
              ...
              return (
                <Card
                  key={task.id}
                  className={`group hover:shadow-md transition-shadow ${config.cardClass}`}
                >
                  <CardHeader>
                    <div className="flex items-start justify-between">
            ...
            <Card className="group hover:shadow-md transition-shadow cursor-pointer border-primary/20 hover:border-primary/40">
              <CardContent className="p-6 flex flex-col items-center text-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Track your grades and calculate GPA</p>
                </div>
